...
Esta entrega preserva integralmente os arquivos V11 de conteúdo e estilo e adiciona autenticação multiusuário, sessões, salvamento granular, controle de versão, indicadores de carregamento e administração. O backend fica em `apps-script/appscript_v13_corrigido.js` e não deve ser publicado em repositório público. Configure `API_URL` em `scripts/app-v13.js` e em `set-password.html`.
