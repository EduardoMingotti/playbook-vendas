...
function validateTokenAndLoad(token){if(!token){showAccessError('Digite a chave de acesso para continuar.');return;}clearAccessError();setAccessLoading(true);return fetchCallsRemote(token).then(data=>{if(!data||data.ok===false)throw new Error(data&&data.error?data.error:'unauthorized');localStorage.setItem(ACCESS_TOKEN_STORAGE_KEY,token);apiReachable=true;applyRemoteCallsToState(data.calls||[]);syncStatus='saved';console.log('Dados carregados V10.3:',(data.calls||[]).length,'calls');unlockApplication();}).catch(err=>{console.error('Falha de autenticação/carregamento V10.3:',err);localStorage.removeItem(ACCESS_TOKEN_STORAGE_KEY);showAccessError('Chave inválida ou backend indisponível. Confira a chave e tente novamente.');}).finally(()=>setAccessLoading(false));}
function loadFromRemote(){const token=getAccessToken();if(!token)return Promise.resolve();return fetchCallsRemote(token).then(data=>{apiReachable=true;if(!data||data.ok===false)throw new Error(data&&data.error?data.error:'Erro ao carregar');applyRemoteCallsToState(data.calls||[]);console.log('Recarregado da nuvem V10.3:',(data.calls||[]).length,'calls');}).catch(err=>{console.error('Não foi possível conectar ao backend V10.3.',err);if(String(err.message).includes('unauthorized'))resetAccessToken();});}
