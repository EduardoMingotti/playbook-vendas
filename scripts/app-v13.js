...
function statusLabel(status){const s=normalizeStatus(status);if(s==='venda')return 'Fechado';if(s==='follow-up')return 'Follow-up';if(s==='no-show')return 'No-show';if(s==='fora de icp')return 'Fora de ICP';if(s==='perdido')return 'Perdido';if(s==='em andamento')return 'Em andamento';return status||'—';}
...
function buildEvolutionData(calls){
  ...
  calls.forEach(c => {
    ...
    const scKeys = (c.status==='No-Show'||c.status==='Fora de ICP') ? [] : Object.keys(c.sc || {});
    if(scKeys.length > 0){
      let sum = 0, count = 0;
      scKeys.forEach(k => {
...
function openEditCallModal(id){
  ...
  const opts=['Em andamento','Follow-up','Venda','Perdido','Fora de ICP','No-Show'].map(s=>'<option value="'+s+'"'+(call.status===s?' selected':'')+'>'+s+'</option>').join('');
...
function submitEditCall(id){
  ...
  const scorecardExempt=call.status==='No-Show'||call.status==='Fora de ICP';
  call.isSao=scorecardExempt?false:document.getElementById('ed-sao').checked;
  if(scorecardExempt&&hasAnyScorecardAnswer(call)){
    const ok=confirm('Este status não utiliza scorecard. Deseja limpar as respostas já preenchidas?');
    if(!ok)return;
    call.sc={};
  }
...
function getDemoRealizadaCalls(calls){ return calls.filter(c=>c.status!=='No-Show'&&c.status!=='Fora de ICP'&&c.status!=='Em andamento'); }
...
function openEndCallModal(){const call=getActiveCall();if(!call)return;ensureCallModel(call);const st=getScorecardStats(call);const scoreMsg=st.complete?`<div class="md-info"><i class="ti ti-check"></i> Scorecard completo: ${st.evaluated}/${st.totalItems} critérios preenchidos.</div>`:`<div class="md-warning"><b>Scorecard pendente:</b> ${st.evaluated}/${st.totalItems} critérios preenchidos. Para concluir como Fechado, Follow-up ou Perdido, complete o scorecard antes de salvar. No-show e Fora de ICP não exigem scorecard.</div>`;const current=call.status&&!isInProgress(call)?call.status:'';showModal('Concluir reunião',`<div class="md-form">${scoreMsg}<div class="md-group"><label class="md-label">Status final da reunião</label><select id="ec-status" class="md-input" onchange="handleEndStatusChange()"><option value="" ${!current?'selected':''}>Selecione o status final...</option><option value="Venda" ${current==='Venda'?'selected':''}>Fechado / venda concluída 🟢</option><option value="Follow-up" ${current==='Follow-up'?'selected':''}>Follow-up agendado 🟡</option><option value="Perdido" ${current==='Perdido'?'selected':''}>Perdido 🔴</option><option value="Fora de ICP" ${current==='Fora de ICP'?'selected':''}>Fora de ICP / sem aderência 🟣</option><option value="No-Show" ${current==='No-Show'?'selected':''}>No-show (lead não compareceu) ⚪</option></select></div><div class="md-group" id="ec-note-wrap"><label class="md-label" id="ec-note-label">Observação da reunião/status</label><textarea id="ec-motivo" class="md-input" rows="3" placeholder="Ex: retorno combinado, motivo da perda, contexto do follow-up...">${escapeHtml(call.statusNote||call.motivoPerdido||'')}</textarea></div><div class="md-group" id="ec-sao-wrap"><label class="md-row-check"><input type="checkbox" id="ec-sao" ${call.isSao?'checked':''} style="accent-color:#3b82f6;" /><span>Marcar como SAO ao final da reunião</span></label></div><div id="ec-scorecard-exempt-warning" class="md-info hidden"></div><div class="md-btns"><button class="btn-secondary" onclick="closeModal()">Cancelar</button><button class="btn-primary" onclick="submitEndCall()">Concluir e salvar</button></div></div>`);handleEndStatusChange();}
function handleEndStatusChange(){const status=document.getElementById('ec-status')?.value;const isIcp=status==='Fora de ICP';const isNoShow=status==='No-Show';const noteWrap=document.getElementById('ec-note-wrap');const noteLabel=document.getElementById('ec-note-label');const note=document.getElementById('ec-motivo');const warning=document.getElementById('ec-scorecard-exempt-warning');if(noteWrap)noteWrap.classList.toggle('hidden',isNoShow);if(noteLabel)noteLabel.textContent=isIcp?'Motivo da desqualificação':'Observação da reunião/status';if(note)note.placeholder=isIcp?'Ex: necessidade principal não atendida, integração inexistente, perfil fora do público-alvo...':'Ex: retorno combinado, motivo da perda, contexto do follow-up...';document.getElementById('ec-sao-wrap')?.classList.toggle('hidden',isNoShow||isIcp);if(warning){warning.classList.toggle('hidden',!isNoShow&&!isIcp);warning.textContent=isIcp?'Fora de ICP não entra como demo válida, não exige scorecard e requer o motivo da desqualificação.':'No-show não entra como demo realizada e não exige scorecard preenchido.';}}
function submitEndCall(){const status=document.getElementById('ec-status').value;if(!status){alert('Selecione o status final da reunião antes de salvar.');return;}const motivo=document.getElementById('ec-motivo').value.trim();const call=getActiveCall();if(!call)return;ensureCallModel(call);const scorecardExempt=status==='No-Show'||status==='Fora de ICP';if(status==='Fora de ICP'&&!motivo){alert('Informe o motivo da desqualificação para concluir como Fora de ICP.');document.getElementById('ec-motivo')?.focus();return;}const st=getScorecardStats(call);if(!scorecardExempt&&!st.complete){alert(`Antes de concluir, preencha o scorecard completo. Hoje estão preenchidos ${st.evaluated}/${st.totalItems} critérios.`);state.active=12;closeModal();saveToStorage();render();window.scrollTo(0,0);return;}const oldStatus=call.status||'';if(scorecardExempt&&hasAnyScorecardAnswer(call)){const label=status==='Fora de ICP'?'Fora de ICP':'No-show';const ok=confirm(`Esta reunião possui scorecard preenchido. Ao salvar como ${label}, as respostas serão limpas e não entrarão nos indicadores. Deseja continuar?`);if(!ok)return;call.sc={};}call.status=status;call.statusNote=motivo;call.isSao=scorecardExempt?false:!!document.getElementById('ec-sao')?.checked;call.motivoPerdido=status==='Perdido'?motivo:'';call.preCallNotes=call.preCallNotes||{text:PRECALL_NOTES_TEMPLATE,updatedAt:'',locked:false};call.preCallNotes.locked=true;call.statusHistory=Array.isArray(call.statusHistory)?call.statusHistory:[];if(oldStatus!==status||motivo){call.statusHistory.push({at:new Date().toISOString(),from:oldStatus,to:status,note:motivo});}state.activeCallId=null;state.active='dashboard';closeModal();saveToStorage();render();}
/* ===== RADAR DE ADERENCIA - 6 PILARES ===== */
const PLAYBOOK_RADAR_PILLARS=[{label:'Abertura',name:'Abertura e controle',criteria:[0]},{label:'Diagnostico',name:'Investigacao e diagnostico',criteria:[1,2,3]},{label:'Valor',name:'Construcao de valor',criteria:[4,5,6]},{label:'Ancoragem',name:'Ancoragem e compromisso',criteria:[7,8]},{label:'Recomendacao',name:'Recomendacao e defesa',criteria:[9,10]},{label:'Fechamento',name:'Fechamento e proximo passo',criteria:[11]}];
function getRadarEligibleCalls(calls){return(Array.isArray(calls)?calls:[]).filter(c=>!['Em andamento','No-Show','Fora de ICP'].includes(c.status)&&getScorecardStats(c).complete);}
function buildPlaybookRadarData(calls){const eligible=getRadarEligibleCalls(calls);return{eligible,pillars:PLAYBOOK_RADAR_PILLARS.map(p=>{let points=0;eligible.forEach(c=>p.criteria.forEach(i=>points+=Number(c.sc&&c.sc[`sc-12-${i}`])||0));const max=eligible.length*p.criteria.length*2;return{...p,points,max,percent:max?Math.round(points/max*100):0};})};}
function appendPlaybookRadar(container,calls){const d=buildPlaybookRadarData(calls),card=mk('div','chart-container');card.style.marginBottom='28px';card.appendChild(mk('div','chart-title','Radar de aderencia ao playbook'));if(!d.eligible.length){card.appendChild(mk('div','evo-note','Nao ha scorecards concluidos nos filtros atuais. No-show, Fora de ICP e reunioes em andamento nao entram neste grafico.'));container.appendChild(card);return;}card.appendChild(mk('div','evo-note',`Baseado em ${d.eligible.length} ${d.eligible.length===1?'reuniao avaliada':'reunioes avaliadas'}.`));const w=mk('div','');w.style='position:relative;height:360px;max-width:760px;margin:0 auto';const cv=document.createElement('canvas');cv.id='chartPlaybookRadar';w.appendChild(cv);card.appendChild(w);const det=mk('div','');det.style='display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:8px 16px;margin-top:14px';d.pillars.forEach(p=>{const r=mk('div','');r.style='padding:9px 10px;border:1px solid #334155;border-radius:8px;background:#111827';r.innerHTML=`<div style="font-size:12px;font-weight:700;color:#e2e8f0">${escapeHtml(p.name)}</div><div style="font-size:11px;color:#94a3b8;margin-top:3px">${p.points} de ${p.max} pontos · ${p.percent}%</div>`;det.appendChild(r);});card.appendChild(det);container.appendChild(card);}
function buildPlaybookRadarChart(calls){if(window.charts.playbookRadar)window.charts.playbookRadar.destroy();const cv=document.getElementById('chartPlaybookRadar');if(!cv)return;const d=buildPlaybookRadarData(calls);if(!d.eligible.length)return;window.charts.playbookRadar=new Chart(cv,{type:'radar',data:{labels:d.pillars.map(p=>p.label),datasets:[{data:d.pillars.map(p=>p.percent),borderColor:'#38bdf8',backgroundColor:'rgba(56,189,248,.22)',pointBackgroundColor:'#38bdf8',pointRadius:3,borderWidth:2}]},options:{responsive:true,maintainAspectRatio:false,scales:{r:{min:0,max:100,ticks:{stepSize:20,color:'#94a3b8',backdropColor:'transparent',callback:v=>v+'%'},grid:{color:'rgba(148,163,184,.20)'},angleLines:{color:'rgba(148,163,184,.20)'},pointLabels:{color:'#cbd5e1',font:{size:11}}}},plugins:{legend:{display:false},tooltip:{callbacks:{label:x=>{const p=d.pillars[x.dataIndex];return`${p.points} de ${p.max} pontos (${p.percent}%)`;}}}}}});}
/* ===== FIM RADAR ===== */
...
function coachIaBuildContextualEvaluationRules(){return `## Contexto e equivalencia - aplicacao obrigatoria

Antes de pontuar, identifique o contexto real: operacao madura ou nova, profissional solo sem sistema proprio, decisor ausente, timing distante, lead exploratorio e aderencia parcial ou limitacao relevante.

Avalie o objetivo comercial do criterio, nao a reproducao literal de uma frase. Quando o criterio nao se aplicar literalmente, procure comportamento equivalente. O contexto altera a evidencia esperada, mas nao elimina a exigencia.

Se o lead nao possui sistema atual, nao exija a pergunta sobre mensalidade. Avalie custo do cenario atual, risco de comecar sem processo, impacto do improviso ou estrutura minima necessaria.

Decisor ausente: avalie identificacao do decisor, criterios de validacao, apoio do interlocutor e proximo passo concreto. "Vou enviar a proposta", sem acao, data, responsavel e criterio, continua incompleto.

Timing distante: nao exija contratacao imediata. Avalie gatilho da decisao, razao da espera, preparacao necessaria e checkpoint coerente. Nao premie urgencia artificial.

Aderencia parcial e Fora de ICP: nao premie insistencia diante de desalinhamento. Avalie honestidade sobre limites, ausencia de promessas indevidas e distincao entre limitacao toleravel e impeditivo. Desqualificacao responsavel pode ser boa execucao. Fora de ICP nao exige scorecard nem aderencia.

Use 0 quando o objetivo nao foi executado; 1 quando foi parcial; 2 quando foi bem executado, literal ou contextualmente. Nao use contexto para justificar ausencia real de investigacao, qualificacao ou conducao.
...
Versao metodologica: 1.1 - avaliacao contextual
...
${coachIaBuildContextualEvaluationRules()}## Fora de ICP e desqualificação responsável

Quando a necessidade principal não possui aderência suficiente à plataforma, classificar ou recomendar Fora de ICP pode representar boa qualificação e prudência comercial, não falha de fechamento.

Nessa situação:

- não exija scorecard nem pontuação de aderência;
- avalie se o closer identificou a limitação;
- verifique se evitou promessas indevidas;
- considere se explicou a aderência real;
- confirme se registrou claramente o motivo da desqualificação.
...
Antes do resumo, informe:

- **Contexto identificado:** operacao madura, operacao nova, profissional solo, decisor ausente, timing distante, aderencia parcial ou outro;
- **Impacto do contexto na avaliacao:** evidencias adaptadas sem eliminar o objetivo dos criterios.
