...
function openEndCallModal(){
  const call=getActiveCall(); if(!call)return; ensureCallModel(call); const st=getScorecardStats(call);
...
function submitEndCall(){
  const status=document.getElementById('ec-status').value; if(!status){alert('Selecione o status final da reunião antes de arquivar.');return;}
  const motivo=document.getElementById('ec-motivo').value.trim(); const call=getActiveCall(); if(!call)return; ensureCallModel(call);
...
function validateTokenAndLoad(token){if(!token){showAccessError('Digite a chave de acesso para continuar.');return;}clearAccessError();setAccessLoading(true);return fetchCallsRemote(token).then(data=>{if(!data||data.ok===false)throw new Error(data&&data.error?data.error:'unauthorized');localStorage.setItem(ACCESS_TOKEN_STORAGE_KEY,token);apiReachable=true;applyRemoteCallsToState(data.calls||[]);syncStatus='saved';console.log('Dados carregados V10.2:',(data.calls||[]).length,'calls');unlockApplication();}).catch(err=>{console.error('Falha de autenticação/carregamento V10.2:',err);localStorage.removeItem(ACCESS_TOKEN_STORAGE_KEY);showAccessError('Chave inválida ou backend indisponível. Confira a chave e tente novamente.');}).finally(()=>setAccessLoading(false));}
function loadFromRemote(){const token=getAccessToken();if(!token)return Promise.resolve();return fetchCallsRemote(token).then(data=>{apiReachable=true;if(!data||data.ok===false)throw new Error(data&&data.error?data.error:'Erro ao carregar');applyRemoteCallsToState(data.calls||[]);console.log('Recarregado da nuvem V10.2:',(data.calls||[]).length,'calls');}).catch(err=>{console.error('Não foi possível conectar ao backend V10.2.',err);if(String(err.message).includes('unauthorized'))resetAccessToken();});}
